...
int main()
{
    ...
    // Освобождаем память и обнуляем указатель, чтобы не остался
    // "болтающийся указатель" (dangling pointer).
    delete[] arr;
    arr = nullptr;

    // Демонстрация безопасного поведения: проверяем указатель на
    // nullptr перед разыменованием, поэтому Segmentation Fault не
    // происходит.
    if (arr == nullptr)
    {
        std::cout << "Указатель arr равен nullptr — доступ к памяти запрещён, "
                     "обращения по нему не выполняем." << std::endl;
    }
    else
    {
        // Эта ветка никогда не выполнится, но показывает, что без
        // проверки на nullptr здесь было бы разыменование
        // освобождённой памяти (неопределённое поведение).
